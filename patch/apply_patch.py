...
# Match MainActivity structurally instead of depending on exact upstream whitespace.
manifest = src / 'AndroidManifest.xml'
s = read(manifest)

main_match = re.search(
    r'<activity\b(?=[^>]*android:name="\.ui\.MainActivity")[\s\S]*?</activity>',
    s,
)
if not main_match:
    raise SystemExit('PATCH FAILED: MainActivity activity not found in AndroidManifest.xml')

main_block = main_match.group(0)
launcher_filter_pattern = re.compile(
    r'\s*<intent-filter>\s*'
    r'<action\s+android:name="android\.intent\.action\.MAIN"\s*/>\s*'
    r'<category\s+android:name="android\.intent\.category\.LAUNCHER"\s*/>\s*'
    r'(?:<category\s+android:name="android\.intent\.category\.LEANBACK_LAUNCHER"\s*/>\s*)?'
    r'</intent-filter>',
    re.MULTILINE,
)
main_without_launcher, removed = launcher_filter_pattern.subn('', main_block, count=1)
if removed != 1:
    raise SystemExit('PATCH FAILED: MainActivity launcher intent-filter not found')
...
replacement = main_without_launcher + aliases
s = s[:main_match.start()] + replacement + s[main_match.end():]
write(manifest, s)
